...
class PatientRequest extends FhirRequest
{
    ...
    public function baseDataRules($prefix): array
    {
        return [
            ...
            $prefix . 'marital_status' => ['nullable', Rule::in(Patient::MARITAL_STATUS_CODE)],
            ...
            $prefix . 'communication' => 'nullable|array',
            $prefix . 'communication.*' => 'required|string|exists:codesystem_bcp47,code',
            ...
            $prefix . 'link' => 'nullable|array',
            $prefix . 'link.*' => 'required|array',
            $prefix . 'link.*.other' => 'required|string',
            $prefix . 'link.*.type' => ['required', Rule::in(Valuesets::LinkType['code'])],
    ...
    public function contactDataRules(): array
    {
        ...
        return array_merge(
            [
                'contact.*.contact_data.relationship' => 'nullable|array',
                'contact.*.contact_data.relationship.*' => ['required', 'string', Rule::in(PatientContact::RELATIONSHIP_CODE)],
                'contact.*.contact_data.name' => 'required|string',
                'contact.*.contact_data.prefix' => 'nullable|string',
                'contact.*.contact_data.suffix' => 'nullable|string',
