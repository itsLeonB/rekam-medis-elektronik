        ...
        <div
            ...
            <div class="md:py-8">
                <span class="inline-flex">
                    <h1 class="text-2xl font-bold text-neutral-black-300">Daftar Account Pasien</h1>
                </span>
                <p class="mb-3 text-base font-normal text-neutral-grey-100">Lihat dan cari <i>Account</i> yang telah
                    dibuat
                </p>
                <Link v-if="['admin', 'keuangan'].includes($page.props.auth.user.roles[0].name)"
                    :href="route('finance.account.new')" as="button"
            ...
            <table class="w-full text-sm text-center rtl:text-right text-neutral-grey-200 ">
                ...
                <tbody v-for="(item, index) in account" :key="index">
                    <tr class="bg-original-white-0 hover:bg-thirdinner-lightteal-300"
                        ...
                        <td class="px-6 py-4 w-3/12">
                            <p>{{ formatTimestamp(item.servicePeriod.start).split('/')[0] }}</p>
                            <p>Jam {{ formatTimestamp(item.servicePeriod.start).split('/')[1] }}</p>
                        </td>
                    </tr>
                </tbody>
            </table>
...
const searchAccount = async () => {
    ...
    switch (searchAttribute.value) {
        ...
        case "status":
            ...
            break;
        case "nama":
            filteredData = originalData.filter(item => item.nama.includes(query))
            account.value = filteredData;
        default:
            break;
    }
