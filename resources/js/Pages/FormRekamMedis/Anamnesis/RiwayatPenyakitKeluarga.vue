    ...
    <div>
        <form @submit.prevent="submit">
            <div class="my-2 w-full" v-for="(field, index) in riwayatPenyakit" :key="index">
                <h3 v-if="index !== 0" class="font-semibold text-secondhand-orange-300 mt-2">Riwayat Penyakit Keluarga {{
                    (index + 1) }}</h3>
                <h3 v-else class="font-semibold text-secondhand-orange-300 mt-2">Riwayat Penyakit Keluarga</h3>
                <div class="flex">
...
const submit = () => {
    riwayatPenyakit.value.forEach(item => {
        ...
        const riwayatPenyakitKeluargaResource = {
        ...
        axios.post(route('integration.store', { resourceType: riwayatPenyakitKeluargaResource.resourceType }), riwayatPenyakitKeluargaResource)
        .then(response => {
