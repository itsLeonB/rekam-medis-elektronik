    ...
    <div>
        <form @submit.prevent="submit">
            <div class="my-2 w-full">
                <h3 class="font-semibold text-secondhand-orange-300 mt-2">Kondisi</h3>
                <div class="flex">
                    <div class="w-full md:w-7/12 mr-2">
                        <InputLabel for="code" value="Kondisi Pasien" />
                        <div class="flex">
                            <Multiselect v-model="resourceForm.code" mode="single" placeholder="Kondisi" :object="true"
                                :options="codeList" label="display" valueProp="code" track-by="code" class="mt-1"
...
const submit = () => {
    const submitResource = {
        ...
        "clinicalStatus": {
            "coding": [
                {
                    "system": resourceForm.value.clinicalStatus.system,
                    "code": resourceForm.value.clinicalStatus.code,
                    "display": resourceForm.value.clinicalStatus.display
                }
            ]
        },
        "category": [
            ...
            "coding": [
                {
                    ...
                    "display": resourceForm.value.code.display
                }
            ]
        },
...
const getCodeList = async () => {
    const { data } = await axios.get(route('terminologi.condition.keluar'));
    codeList.value = data;
};
